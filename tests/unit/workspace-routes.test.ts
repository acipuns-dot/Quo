...
describe("workspace route handlers", () => {
  ...
  it("accepts draft saves when an additional fee amount is filled before its label", async () => {
    process.env.NEXT_PUBLIC_SUPABASE_URL = "https://example.supabase.co";
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY = "anon-key";
  ...
  it("still rejects exported saves when an additional fee label is blank", async () => {
    process.env.NEXT_PUBLIC_SUPABASE_URL = "https://example.supabase.co";
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY = "anon-key";
    ...
    ).rejects.toThrow("Fee label is required.");
